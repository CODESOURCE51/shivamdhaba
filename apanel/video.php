...
$query_update= "insert into `video_url` SET 
...
$query_update= "UPDATE `video_url` SET 
            `video_id`='".$_REQUEST['video_id']."' where id='".$_REQUEST['id']."'";
			
 $qu_up= mysql_query($query_update) or die(mysql_error());	

echo "<script type='text/javascript'> window.location= 'video.php?updated'; </script>";
}

if(isset($_REQUEST['action']) && $_REQUEST['action']=='del'){
	
$query_exi=mysql_query("delete from `video_url` where id='".$_REQUEST['id']."'");

echo "<script type='text/javascript'> window.location= 'video.php?deleted'; </script>";
...
<?php if(isset($_REQUEST['updated'])) { echo "<div style='color:#090;font-size:16px;'>Thanks! Record updated successfully.</div>"; } ?>
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="change_password" id="change_password" enctype="multipart/form-data" >
	...
	$fpro= mysql_fetch_array(mysql_query("select * from sub_category where id='".$_REQUEST['edit']."'"));?>
    ...
    <?php }?>
      ...
      <tr>
        <td height="40" colspan="3" align="center" valign="middle"  style="font-family:Verdana, Geneva, sans-serif; font-size:22px;">Add Video Url</td>
        </tr>
      <tr>
        <td width="137" height="35" align="left" valign="middle" class="master">Video ID</td>
        <td width="21" height="35" align="center" valign="middle">:</td>
        <td width="292" height="35" align="left" valign="middle" class="error">
        <input type="text" name="video_id" id="video_id" class="rounded" value="<?php if(isset($_REQUEST['edit'])){ echo $fpro['video_id'];}?>"/>
       </td>
        </tr>
      <tr>
...
<tr>
      ...
      <tr>
        <td height="40" colspan="2" align="center" valign="middle"  style="font-family:Verdana, Geneva, sans-serif; font-size:22px; border-bottom:#3B7F95 1px solid;">Video Link List</td>
        </tr>
      <tr>
        <td height="30" align="left" valign="bottom" style="font-family:Verdana, Geneva, sans-serif; font-size:16px; border-bottom:#3B7F95 1px solid;">Video ID</td>
...
<?php
	$adjacents = 3;			
	$query = "SELECT COUNT(*) as num FROM video_url";
	...
	$targetpage = "video.php"; 	//your file name  (the name of this file)
	...
	$sql = "select * from video_url order by id desc LIMIT $start, $limit";
	...
	*/
...

	{
	 ...
	 // while($pro= mysql_fetch_array($f_pro)){
	  ...
	  ?>
      <tr>
        <td height="20" align="left" valign="middle"><?php echo $row['video_id'] ;?></td>
        <td height="20" align="left" valign="middle">
        <a href="video.php?action=new&edit=<?php echo $row['id'] ;?>"><img src="img/edit_icon.png"></a>|
        <a href="video.php?action=del&id=<?php echo $row['id'] ;?>"><img src="img/del_icon.png"></a>
         </td>
        </tr>
      <?php }}?>
        <tr>
